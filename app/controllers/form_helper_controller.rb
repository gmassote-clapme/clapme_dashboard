class FormHelperController < ApplicationController
  ...
  protected
    ...
    def build_date_time(param_base, param, param_date, param_time)
      ...
      param_base_hash[param] = Time.strptime("#{param_base_hash[param_date]} #{param_base_hash[param_time]}", '%d/%m/%Y %H:%M') unless param_base_hash[param_date].blank? and param_base_hash[param_time].blank?
    end
